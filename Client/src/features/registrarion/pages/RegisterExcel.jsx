...
const RegisterExcel = () => {
  ...
  const validateOptionalProfessorFields = (row) => {
    ...
    const allEmpty = professorFields.every(
      (field) => !field || String(field).trim() === ""
    );
    const allFilled = professorFields.every(
      (field) => field && String(field).trim() !== ""
    );
  ...
  const normalizeTutorType = (value) => {
    ...
    if (parentVariations.some((v) => normalized.includes(v)))
      return "MAMÁ/PAPÁ";
    if (tutorVariations.some((v) => normalized.includes(v)))
      return "TUTOR LEGAL";
          ...
          if (row.length > 16) {
            ...
            if (!["MAMÁ/PAPÁ", "TUTOR LEGAL"].includes(normalizedValue)) {
              errors.push({
                ...
                column: 17,
                header: headers[16],
                message: `Fila ${rowIndex + 2}, Columna 17 (${
                  headers[16]
                }): Valor inválido (debe ser MAMÁ/PAPÁ o TUTOR LEGAL)`,
              });
          ...
          row.forEach((cell, cellIndex) => {
            ...
            if (cellIndex < 17 && !cellValue) {
              errors.push({
                ...
                column: cellIndex + 1,
                header: header,
                message: `Fila ${rowIndex + 2}, Columna ${
                  cellIndex + 1
            ...
            if (cellValue) {
              ...
              if (!isValid) {
                errors.push({
                  ...
                  message: `Fila ${rowIndex + 2}, Columna ${
                    cellIndex + 1
                  } (${header}): ${getErrorMessage(errorType)}`,
                });
                newCellErrors[`${rowIndex}-${cellIndex}`] = true;
              }
            }
  ...
  const getErrorMessage = (errorType) => {
    switch (errorType) {
      case "invalid_carnet":
        return "Carnet inválido (6-12 caracteres alfanuméricos)";
      case "invalid_date":
        return "Fecha inválida";
      case "invalid_email":
        return "Correo electrónico inválido";
      case "invalid_phone":
        return "Celular inválido (8 dígitos exactos)";
      case "invalid_tutor":
        return "Valor inválido (MAMÁ/PAPÁ o TUTOR LEGAL)";
      case "columns":
        return "Número incorrecto de columnas";
      default:
  ...
  const isProfessorRowComplete = (row) =>
    row.slice(17, 22).some((field) => field);
