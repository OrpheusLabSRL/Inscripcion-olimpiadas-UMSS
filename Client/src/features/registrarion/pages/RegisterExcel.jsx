...
const RegisterExcel = () => {
  ...
  const normalizeCourse = (input) => {
    ...
    const exactMatch = validCourses.find(
      (course) => course.toLowerCase() === cleanInput
    );
    ...
    const flexibleMatch = validCourses.find((course) => {
      ...
      const inputWithoutSymbol = cleanInput
        .replace(/º|°/g, "")
        .replace(/([0-9])([a-z])/, "$1 $2");
      return (
        courseLower === inputWithoutSymbol ||
  ...
  const normalizeArea = (input) => {
    ...
    const matchedArea = validAreas.find(
      (area) =>
        area
          .toLowerCase()
          .normalize("NFD")
  ...
  const validateProfessorFields = (row, rowIndex) => {
    ...
    if (hasSomeFields && !hasAllFields) {
      return `Fila ${
        rowIndex + 2
      }: Los campos de PROFESOR deben estar todos completos o todos vacíos`;
    }
    return null;
  };
      ...
      if (row[9]) {
        ...
        if (normalizedArea) {
          ...
          errors.push(
            `Fila ${
              rowIndex + 2
            }: AREA no válida. Las áreas válidas son: ${validAreas.join(", ")}`
          );
      ...
      if (row[10]) {
        ...
        if (normalizedCategory) {
          ...
          errors.push(
            `Fila ${
              rowIndex + 2
            }: CATEGORIA no válida. Las categorías válidas son: ${validCategories.join(
              ", "
            )}`
          );
      ...
      if (row[8]) {
        ...
        if (normalizedCourse) {
          ...
          errors.push(
            `Fila ${
              rowIndex + 2
            }: CURSO (OLIMPISTA) no válido. Los cursos válidos son: ${validCourses.join(
              ", "
            )}`
          );
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        setError(
          `Se encontraron ${errors.length} errores de validación. Revise los campos marcados en rojo.`
        );
      } else {
        setSuccess(
      ...
      if (response.success) {
        ...
        if (response.data.errores?.length > 0) {
          ...
          if (response.data.errores.length > 5) {
            successMsg += `\n...y ${
              response.data.errores.length - 5
            } errores más.`;
          }
        }
        ...
        swal({
          title:
            response.data.errores?.length > 0 ? "Registro parcial" : "Éxito",
          text: successMsg,
