...
export const OCRValidation = () => {
  ...
  const checkCodigoBoleta = async (codigo) => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/boletaPago/check",
        {
          method: "POST",
          headers: {
  ...
  const confirmarPago = async (codigo) => {
    if (!codigo) {
      alert(
        "Código de boleta no detectado. Por favor, procese una imagen válida."
      );
      return;
    }
    if (boletaPaid) {
    ...
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/boletaPago/confirmarPago",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="ocrvalidation-container">
      ...
      <div className="ocrvalidation-panel" style={{ marginTop: "20px" }}>
        ...
        {ocrResult && (
          ...
          >
            <div>
              <strong>Código de Boleta detectado:</strong>{" "}
              <span>{codigoBoleta ? codigoBoleta : "N/A"}</span>
            </div>
            <div>
        ...
        {codigoBoleta && (
          <>
            <button
              onClick={() => confirmarPago(codigoBoleta)}
              style={{
                marginTop: "10px",
                marginRight: "10px",
                width: "150px",
